...
function rawLanguageDataFn(d) {
    ...
    return nestedGet(d, ['data', 'loc', 'language']);
}

function languageDataFn(d) {
    return nestedGet(d, ['data', 'data', 'loc', 'language']);
}
...
function ageDataFn(d) {
    return nestedGet(d, ['data', 'data', 'git', 'age_in_days']);
}

function authorsDataFn(d) {
    return nestedGet(d, ['data', 'data', 'git', 'user_count']);
}
...
function buildColourStrategies(rootConfig, languageColours) {
    ...
    return {
        age: {
            fillFn: buildScaledNodeColourFn(ageDataFn,
...
function buildHierarchy(data) {
    return d3.hierarchy(data, d => d.children)
        .sum((d) => {
            if (d.data === undefined || d.data.loc === undefined) {
                return 1;
            }
...
function nodeHtmlDetails(node) {
    const pathBits = node.data.path.split('/');
    const pathBitsHtml = pathBits.map((name, index) => {
        const pathSoFar = pathBits.slice(0, index + 1);
        return `<button class="zoomer" data-path="${pathSoFar.join('/')}">-></button> ${name}`;
    })
        .join('<br/>');

    const age = ageDataFn(node);
    // TODO: distinguish between "never changed" and "not in git"
    const ageHtml = age === undefined ? '' : `<p>${age} days since changed</p>`;
...
function updateChart(config, elements, state, selectNodeCallback) {
    ...
    const rootNode = treeNodeByPath(state.chartData, state.rootNodePath);
...
function onNodeClickedFn(config, chartState, refreshFn) {
    return (node, i, nodes) => {
        ...
        const inspector = document.getElementById('inspector');
        // TODO: restore URL for node
        inspector.innerHTML = nodeHtmlDetails(node);
...
d3.json("js/data/flare.json", (data) => {
    addPaths(data, null);
    languageCounts = countLanguagesIn(data);
    languageColours = buildLanguageColours(languageCounts);
    strategies = buildColourStrategies(chartConfig, languageColours);

    globalChartState.chartData = data;
    globalChartState.currentStrategy = strategies.language;
    refresh();
})
